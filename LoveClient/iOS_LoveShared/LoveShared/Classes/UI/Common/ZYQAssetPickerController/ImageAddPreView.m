...
- (void)initResource
{
    ...
    [_selectBeforeLab setBackgroundColor:[UIColor clearColor]];
    ...
    [_selectafterLab setBackgroundColor:[UIColor clearColor]];
    [self addSubview: _selectafterLab];
    [_selectafterLab setTextAlignment:NSTextAlignmentRight];
    [_selectafterLab setText:@"张照片"];
    
    _startPintuButton =  [[UIButton alloc] initWithFrame:CGRectMake(self.bounds.size.width - 108/2.0f-10,
                                                                    10,
                                                                    108/2.0f,
                                                                    56/2.0f)];
...
//        [editView addGestureRecognizer:longGesture];
    ...
    else if (sender.state == UIGestureRecognizerStateChanged)
    {
        ...
        else
        {
            [UIView animateWithDuration:Duration animations:^{
                ...
                UIButton *button = _imageEditViewArray[index];
                temp = button.center;
...
{
    for (NSInteger i = 0;i<_imageEditViewArray.count;i++)
    {
        UIButton *button = _imageEditViewArray[i];
        if (button != btn)
        {
