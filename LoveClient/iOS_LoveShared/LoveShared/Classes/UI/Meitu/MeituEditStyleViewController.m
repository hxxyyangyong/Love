...
- (void)initResource
{
    ...
    self.freeView = [[GLMeituFreeContentView alloc] initWithFrame:CGRectMake(self.contentView.frame.size.width,
                                                                             0,
                                                                             self.contentView.frame.size.width,
                                                                             self.contentView.frame.size.height)];
...
- (void)didSelectedWithPoster:(NSDictionary *)posterDict isEmpty:(BOOL)isEmpty
{
    ...
    if (isEmpty) {
        ...
        if ([posterDict objectForKey:@"PosterImagePath"]) {
            posterImage = [UIImage imageNamed:[posterDict objectForKey:@"PosterImagePath"]];
            //[UIImage imageWithContentsOfFile:[posterDict objectForKey:@"PosterImagePath"]];
...
- (void)didSelectedWithBorder:(NSDictionary *)borderDict isEmpty:(BOOL)isEmpty
{
    if (isEmpty) {
        ...
        if ([borderDict objectForKey:@"BorderImagePath"]) {
            UIImage *posterImage = [UIImage imageNamed:[borderDict objectForKey:@"BorderImagePath"]];
            //[UIImage imageWithContentsOfFile:[borderDict objectForKey:@"BorderImagePath"]];
...
- (void)initPosterView
{
    _posterView = [[GLPosterSelectView alloc] initWithFrame:CGRectMake(self.bottomControlView.frame.size.width, 0, self.bottomControlView.frame.size.width, self.bottomControlView.frame.size.height)];
    [_posterView setBackgroundColor:[[UIColor colorWithHexString:@"#454545"] colorWithAlphaComponent:0.6]];
    _posterView.delegateSelect = self;
    [_bottomControlView addSubview:_posterView];
}
...
#pragma mark 
#pragma mark 选择默认的自由模式底纹
- (void)didSelectedPosterStyleIndex:(NSInteger)styleIndex
{
    if (styleIndex != _posterView.iconCount) {
        [_freeView setBackgroundImage:[UIImage imageNamed:[NSString stringWithFormat:@"poster_style_%d.jpg",styleIndex]]];
    }else{
//        GLBaseSelectBackgroundViewController *gcVC = [[GLBaseSelectBackgroundViewController alloc] init];
//        gcVC.resourceType = E_ResourceType_MeituBackground;
//        gcVC.isHasEmpty = YES;
//        gcVC.delegateSelectPet = self;
//        gcVC.blurImage = [ImageUtility cutImageWithView:self.view];
...
#pragma mark
#pragma mark 选择更多的自由模式的底纹 -GLBaseSelectBackgroundViewControllerDelegate
//- (void)didSelectedWithPet:(NSDictionary *)petDict resourceInfo:(GLEntity_Resource *)resourceInfo isEmpty:(BOOL)isEmpty
//{
//    if (isEmpty) {
//        [self.freeView setBackgroundImage:nil];
//    }else{
//        if (petDict) {
//            if ([petDict objectForKey:@"BackImageFilePath"]) {
//                [self.freeView setBackgroundImage:[UIImage imageWithContentsOfFile:[petDict objectForKey:@"BackImageFilePath"]]];
//            }
//        }
//    }
//}
...
- (CGRect)rectWithArray:(NSArray *)array andSuperSize:(CGSize)superSize
{
    ...
    rect = [GLSmallPhotoInfo rectScaleWithRect:rect scale:2.0f];
...
{
    if (_freeView.assets == nil) {
        [_freeView setAssets:self.assets];
        [[LoadingViewManager sharedInstance] showLoadingViewInView:self.view withText:@"正在处理"];
        dispatch_async(dispatch_get_main_queue(), ^{
            [_freeView initData];
            [_contentView setContentOffset:CGPointMake(_contentView.frame.size.width, 0) animated:YES];
            [[LoadingViewManager sharedInstance] removeLoadingView:self.view];
        });
    }else{
        [self.contentView setContentOffset:CGPointMake(self.contentView.frame.size.width, 0) animated:YES];
    }
}
